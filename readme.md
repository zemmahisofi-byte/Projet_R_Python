# Tremor Analysis using Accelerometer Data
## Author
Sofia Zemmahi

## Project Description
This project aims to analyze tremor signals using smartphone accelerometer data. The objective is to estimate tremor frequency and compare conditions (e.g., before vs after).

## Workflow
1. Data acquisition using smartphone (Phyphox)
2. Signal preprocessing (Python)
3. Frequency analysis (Python)
4. Visualization and comparison (R)

## Project Structure
- `data/` : raw and processed data
- `sources/` : Python and R scripts
- `notebooks/` : analysis notebooks
- `results/` : figures and extracted features

## Tools
- Python (NumPy, Pandas, SciPy)
- R (visualization and statistical analysis)
